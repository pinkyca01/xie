    ...
    if (found === false) {
     ...
     if (isGroup) {
      ...
      if (budy.startsWith('>')){
        ...
        if (budy.startsWith('=>')){
        if (!isOwner && !caa.key.fromMe) return
        var konsol = budy.slice(3)
        return = (fxsxgans) => {
        var nuycans = JSON.stringify(fxsxgans, null, 2)
        xieca = util.format(nuycans)
        if (nuycans == undefined){
        xieca = util.format(fxsxgans)
        }
        return replyca(xieca)
        }
        try {
        replyca(util.format(eval(`;(async () => { ${konsol} })()`)))
        console.log('\x1b[1;37m>', '[', '\x1b[1;32mEXEC\x1b[1;37m', ']', time, color(">", "aqua"), 'from', color(sender.split('@')[0]), 'args :', color(args.length))
        } catch(e){
        replyca(String(e))
        }
        }
...
// Anime
            ...
            case 'hentai4everyone':
               ...
               nuy.sendMessage(from, ini_buffer, image, {quoted: caa})
               await limitAdd(sender)
               break
            case 'bj':
            case 'ero':
